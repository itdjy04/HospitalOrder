...
import edu.jssvc.dao.CommonUserDao;
import edu.jssvc.entity.CommonUser;
import edu.jssvc.service.CommonUserService;
import edu.jssvc.utils.DateUtil;
import edu.jssvc.utils.DateUtil.DateFormat;

import edu.jssvc.utils.GetIP;
import edu.jssvc.utils.MD5;
import edu.jssvc.utils.MailUtil;
...
public class CommonUserServiceImpl implements CommonUserService {
	...
	public int login(String userEmail, String userPassword, HttpServletRequest request) {
		...
		if (commonUser == null) {
			...
			if (MD5.getMD5(userPassword).equalsIgnoreCase(commonUser.getUserPassword())) {
	...
	public boolean sendEmailCheck(CommonUser commonUser) {
		...
		log.info(email);
		String sender = "苏州市医院预约系统";
		String title = "苏州市医院预约挂号【验证码】";
	...
	/**
	 * 管理员登录 0:用户不存在 1:密码不正确 2:不是管理员 3:登录成功
	 */
	@Override
	public int loginAdmin(String userEmail, String userPassword, HttpServletRequest request) {
		CommonUser commonUser = commonUserDao.findCommonUserByEmail(userEmail);
		if (commonUser == null) {
			return 0; // 用户不存在
		}
