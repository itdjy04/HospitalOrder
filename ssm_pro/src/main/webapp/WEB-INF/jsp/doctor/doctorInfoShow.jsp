...
<script src="${mybasePath}assets/date/js/index.js?v=2"></script>
<script>
// 医生排班数据初始化 - 从服务端获取真实排班数据
var doctorId = ${doctor.id};
$(function(){
	$.get('${mybasePath}getScheduleByDoctor', {doctorId: doctorId}, function(data){
		if(data && data.slots && data.slots.length > 0){
			// 将排班数据转换为日历组件需要的格式
			var dateMap = {};
			for(var i = 0; i < data.slots.length; i++){
				var slot = data.slots[i];
				if(!dateMap[slot.scheduleDate]){
					dateMap[slot.scheduleDate] = {s: false, z: false, x: false};
				}
				if(slot.timeSlot === '8:00-11:00') dateMap[slot.scheduleDate].s = true;
				if(slot.timeSlot === '13:00-15:00') dateMap[slot.scheduleDate].z = true;
				if(slot.timeSlot === '15:00-18:00') dateMap[slot.scheduleDate].x = true;
			}
			// 重建日历：重设 div 后先构造 Datepicker（不自动渲染），
			// 填充 PArr 后再调用 initial() 一次性完成渲染+标记
			$('.a').html('');
			var myDate = new Date();
			var y = myDate.getFullYear();
			var m = myDate.getMonth() + 1;
			var d = myDate.getDate();
			var dp = new Datepicker($('.a'), y, m, d);
			var keys = Object.keys(dateMap);
			for(var k = 0; k < keys.length; k++){
				var dateKey = keys[k];
				dp.PArr.push(new specialDate(dateKey, dateMap[dateKey]));
			}
			dp.initial();
		} else {
			$('.a').html('<div class="alert alert-warning" style="padding:20px;">该医生暂无排班信息，请稍后再试或选择其他医生。</div>');
		}
	}, 'json').fail(function(){
		$('.a').html('<div class="alert alert-danger" style="padding:20px;">加载排班数据失败，请刷新页面重试。</div>');
	});
});
</script>
</head>
				...
				<div id="myTabContent" class="tab-content">
					<c:if test='${hos.isOpen=="1" }'>
						...
						<div class="tab-pane fade text-left" id="orderNotice">
							...
							<p>
								<br> <strong>电话预约挂号：0512-0512（24小时）<br>
